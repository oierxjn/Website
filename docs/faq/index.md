...
2. 用 VS 执行（Ctrl+F5/F5）并快速关闭了命令行窗口（实际程序运行之前）。此时需要从任务管理器的后台进程中找到对应的程序（可以点击状态栏排序，将挂起状态排在前面），右键将其结束。
...
当VS控制台未启动程序便结束之后，很快就会发出创建调试进程的信号。此时关闭控制台，调试进程还未初始化，VS将会判定为调试控制台未正常启动（实际上是安全软件正在检查程序，将其进程创建阻塞），弹出上图弹窗。由于父主进程被结束，清理没有执行。500ms-2000ms左右安全程序检查完成之后调试进程初始化并完成，但由于没有进程调用，便作为残留进程挂起，需要手动结束。

上述过程为使用ProcMon分析结果，供参考。

挂起进程的程序保持只读状态。

:::

![点击"状态栏"排序找到对应的挂起进程](https://cdn.jsdelivr.net/gh/ptezhub/ptezimg001@main/2026/20260420223004634.png)
